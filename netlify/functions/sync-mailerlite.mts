...
export default async (req: Request) => {
  ...
  const expectedSecret = Netlify.env.get("MAILERLITE_WEBHOOK_SECRET");
  ...
  const result = await syncContactSubmission(body, true);

  return json(result.payload, result.status);
};

async function syncContactSubmission(body: any, includeGoogleSheets: boolean) {
  ...
  if (!apiToken || !groupId || (includeGoogleSheets && !googleSheetsWebhookUrl)) {
    return {
      status: 500,
  ...
  const sheetsResult = includeGoogleSheets && googleSheetsWebhookUrl
    ? await syncGoogleSheets(googleSheetsWebhookUrl, submission)
    : { ok: true, status: 200 };
